...
var logger = xlog.NewPackageLogger("github.com/effective-security/promptviser/internal/adviserdb", "pgsql")
...
func DbMeasureQuerySince(query string, start time.Time) {
	...
	if elapsedMS > int64(DbSlowMethodMilliseconds) {
		logger.KV(xlog.DEBUG, "reason", "slow", "db", "adviserdb", "query", query, "ms", elapsedMS)
	}
